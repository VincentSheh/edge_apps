...
app.controller('DashController', ['$scope', '$http', '$window', 'sources', 'contributors', 'dashifTestVectors', function ($scope, $http, $window, sources, contributors, dashifTestVectors) {
    $scope.selectedItem = {
    ...
    $scope.chartState = {
        audio: {
            ...
            stallDuration:{ data: [], selected: false, color: '#65080c', label: 'Stalling Duration' },
            stallCount:{ data: [], selected: false, color: '#65080c', label: 'Stalling Count' },
    ...
    // Custom Metrics
    $scope.initialStartupDelay = 0;
    $scope.totalStallDuration = 0;
    $scope.stallCount = 0;
    ...
    $scope.player.on(dashjs.MediaPlayer.events.QUALITY_CHANGE_RENDERED, function (e) {
        ...
        // Custom
        // var vqm = getVQM(e.newQuality);
        // vqmValues.push(vqm);
        // N = vqmValues.length;
        
        // // Calculate P1
        // P1 = vqmValues.reduce((sum, vqm, index) => sum + vqm * Math.exp(0.02 * index * 1), 0) / N;
  
        // // Calculate P2
        // if (N > 1) {
        //   P2 = vqmValues.slice(0, -1).reduce((sum, vqm, index) => 
        //     sum + Math.pow(Math.abs(vqm - vqmValues[index + 1]), 2) * Math.sign(vqmValues[index + 1] - vqm), 0) / (N - 1);
        // } else {
        //   P2 = 0;
        // }
        ...
        // console.log('P1:', P1);
        // console.log('P2:', P2);
        // console.log('Impairment of Level Variation (ILV):', ILV);
    ...
    // Custom
    $scope.player.on(dashjs.MediaPlayer.events.PLAYBACK_STARTED, function (e) {
        var now = new Date().getTime() / 1000;
        if ($scope.initialStartupDelay === 0) {
            $scope.initialStartupDelay = now - $scope.sessionStartTime;
            $scope.safeApply(); // Apply changes to the scope
            console.log('Initial Startup Delay:', $scope.initialStartupDelay, 'seconds');
        }
    }, $scope);
    var stallStartTime = null;
    $scope.player.on(dashjs.MediaPlayer.events.BUFFER_EMPTY, function (e) {
        stallStartTime = performance.now(); // Capture the stall start time
        $scope.safeApply();
    }, $scope);
    
    $scope.player.on(dashjs.MediaPlayer.events.BUFFER_LOADED, function (e) {
        if (stallStartTime) {
          var stallEndTime = performance.now(); // Capture the stall end time
          var stallDuration = stallEndTime - stallStartTime; // Calculate the stall duration
          $scope.totalStallDuration += stallDuration; // Accumulate total stall duration
          $scope.stallCount += 1; // Increment stall count
          stallStartTime = null; // Reset the stall start time
          $scope.safeApply();
        }
    }, $scope);        
    ...
    $scope.getMetrics = function() {
        console.log("TEST \n", $scope.chartState.video);
        console.log($scope.initialStartupDelay);
        
        // Store metrics in a global variable
        window.metrics = {
            videoChartState: $scope.chartState.video,
            initialStartupDelay: $scope.initialStartupDelay
        };
    };

    $scope.sendQoEMetrics = function() {
        var qoeMetrics = {
            initial_delay: $scope.initialStartupDelay, // 
            total_stall_duration: $scope.chartState.video.totalStallDuration.data,
            stall_count: $scope.chartState.video.stallCount.data,
            bitrate: $scope.chartState.video.bitrate.data, // Assuming this is tracked elsewhere in your code
        };
        console.log("Run HTTP POST...")
        $http.post('http://localhost:5001/calculate_qoe', qoeMetrics)
            .then(function(response) {
                console.log('QoE Score:', response.data.qoe_score);
                // $scope.qoeScore = response.data.qoe_score;
            }, function(error) {
    ...
    function updateMetrics(type) {
        ...
        if (dashMetrics && $scope.currentStreamInfo) {
            ...
            var stallDuration = $scope.totalStallDuration;
            var stallCount = $scope.stallCount;
            ...
            $scope[type + 'stallDuration'] = stallDuration;
            $scope[type + 'stallCount'] = stallCount
            ...
            if ($scope.chartCount % 2 === 0) {
                ...
                $scope.plotPoint('stallDuration', type, stallDuration, time);
                $scope.plotPoint('stallCount', type, stallCount, time);
